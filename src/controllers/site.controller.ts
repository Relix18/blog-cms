...
export const updateSiteSettings = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!logo.startsWith("https://res.cloudinary.com")) {
      await cloudinary.uploader.destroy(settings?.logoUrlId as string);
...
export const updateSiteHeroImage = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (settings?.heroImageUrl) {
      if (settings?.heroImageUrlId) {
        await cloudinary.uploader.destroy(settings.heroImageUrlId as string);
      }
